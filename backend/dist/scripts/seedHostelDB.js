...
exports.runHostelSeed = runHostelSeed;
...
const HOSTEL_INFO_CSV = path_1.default.resolve(__dirname, '../../Hostel Info.csv');
const PARENT_CREDS_CSV = path_1.default.resolve(__dirname, '../../Parent Credentials.csv');
...
async function runHostelSeed(isDryRun = true) {
    ...
    const infoContent = fs_1.default.readFileSync(HOSTEL_INFO_CSV, 'utf8');
    const infoLines = infoContent.split(/\r?\n/).filter((l) => l.trim().length > 0);
    const studentRecordsToCreate = [];
    const parentRecordsToCreate = [];
    const usnSet = new Set();
    const duplicateUSNs = [];
    const parentEmailSet = new Set();
    const duplicateParentEmails = [];
    ...
    for (let attempt = 1; attempt <= 5; attempt++) {
        try {
            await mongoose_1.default.connect(mongoURI, { dbName: targetDbName, serverSelectionTimeoutMS: 15000 });
            connected = true;
            break;
        }
        catch (err) {
    ...
    if (!connected)
        throw new Error('Failed to connect to MongoDB Atlas after 5 attempts.');
    ...
    for (const s of staffAccountsToCreate) {
        const existing = await User_1.User.findOne({ email: s.email.toLowerCase() });
        if (!existing) {
            const hashedPassword = await bcryptjs_1.default.hash(s.rawPassword, 10);
    ...
    for (const st of studentRecordsToCreate) {
        const existing = await User_1.User.findOne({ usn: st.usn });
        const hashedPassword = await bcryptjs_1.default.hash(st.rawPassword, 10);
        if (!existing) {
            await User_1.User.create({
                usn: st.usn,
    ...
    for (const p of parentRecordsToCreate) {
        const existing = await User_1.User.findOne({ email: p.email.toLowerCase(), role: 'parent' });
        const hashedPassword = await bcryptjs_1.default.hash(p.rawPassword, 10);
        if (!existing) {
            await User_1.User.create({
                name: p.name,
