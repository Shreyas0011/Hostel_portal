...
if (!content.includes('viewAttendanceStudentId')) {
...
// 2. Add Health and Attendance Helpers
const healthAndAttendanceHelpers = `
...
if (!content.includes('renderWardenAttendanceView')) {
  content = content.replace('// View template: WARDEN DASHBOARD', healthAndAttendanceHelpers + '\n// View template: WARDEN DASHBOARD');
}

// 3. Update Student View Nav & Content
if (!content.includes('data-tab="health"')) {
  content = content.replace(
  ...
  content = content.replace(
    "${state.studentActiveTab === 'leave' ? 'Leave Requests' : 'Report Complaint'}",
    "${state.studentActiveTab === 'leave' ? 'Leave Requests' : state.studentActiveTab === 'complaints' ? 'Report Complaint' : 'Health Status'}"
  );
  content = content.replace(
    "state.studentActiveTab === 'leave' ? renderLeaveSection(student, 'student') : \n          renderComplaintsSection(student)",
    "state.studentActiveTab === 'leave' ? renderLeaveSection(student, 'student') : \n          state.studentActiveTab === 'complaints' ? renderComplaintsSection(student) : renderHealthStatusSection(student, 'student')"
  );
}

// 4. Update Parent View Nav & Content
if (!content.includes("data-tab=\"health\"") || content.includes("data-tab=\"attendance\"") && !content.split("data-tab=\"health\"").length > 2) {
  content = content.replace(
  ...
  content = content.replace(
    "state.parentActiveTab === 'meals' ? \"Child's Dining Planner\" : \"Attendance & History\"",
    "state.parentActiveTab === 'meals' ? \"Child's Dining Planner\" : state.parentActiveTab === 'attendance' ? \"Attendance & History\" : \"Health Records\""
  );
  content = content.replace(
    "state.parentActiveTab === 'meals' ? renderMealsPlanner(student, true) :\n          renderParentAttendanceSection(student)",
    "state.parentActiveTab === 'meals' ? renderMealsPlanner(student, true) :\n          state.parentActiveTab === 'attendance' ? renderParentAttendanceSection(student) : renderHealthStatusSection(student, 'parent')"
  );
}

// 5. Update Warden
if (!content.includes("wardenActiveTab === 'attendance'")) {
  content = content.replace(
  ...
  content = content.replace(
    "state.wardenActiveTab === 'beds' ? 'Room & Bed Assignments' : 'Student Directory'",
    "state.wardenActiveTab === 'beds' ? 'Room & Bed Assignments' : state.wardenActiveTab === 'attendance' ? 'Gate & Attendance' : state.wardenActiveTab === 'health' ? 'Health & Medical Logs' : 'Student Directory'"
  );
  content = content.replace(
...
// 6. Update Admin
if (!content.includes("adminActiveTab === 'attendance'")) {
  content = content.replace(
  ...
  content = content.replace(
    "state.adminActiveTab === 'leaves' ? 'Student Absence Registry' : 'Student Directory'",
    "state.adminActiveTab === 'leaves' ? 'Student Absence Registry' : state.adminActiveTab === 'attendance' ? 'Gate & Attendance' : state.adminActiveTab === 'health' ? 'Health & Medical Logs' : 'Student Directory'"
  );
  content = content.replace(
...
// 7. Update Superadmin
if (!content.includes("superActiveTab === 'attendance'")) {
  content = content.replace(
  ...
  content = content.replace(
    "state.superActiveTab === 'directory' ? 'Student Directory & Gate Logs'",
    "state.superActiveTab === 'directory' ? 'Student Directory' : state.superActiveTab === 'attendance' ? 'Gate & Attendance Logs' : state.superActiveTab === 'health' ? 'Health & Medical Logs'"
  );
  content = content.replace(
    "state.superActiveTab === 'directory' ? renderWardenDirectory() : state.superActiveTab === 'logs'",
    "state.superActiveTab === 'directory' ? renderWardenDirectory() : state.superActiveTab === 'attendance' ? renderWardenAttendanceView() : state.superActiveTab === 'health' ? renderWardenHealthView() : state.superActiveTab === 'logs'"
  );
}

// 8. Add buttons to directory table
const tableBtnTarget = '<button class="table-btn btn-view-student" data-stu-id="${s.id}">View Log</button>';
if (content.includes(tableBtnTarget)) {
  content = content.replace(
    tableBtnTarget,
...
// 9. Inject Event Listeners for the buttons inside btn-view-student block
const dirEventHandlers = `document.querySelectorAll('.btn-view-health').forEach(btn => {
...
// Replace all occurrences of btn-view-student event listener
content = content.replaceAll(
  "document.querySelectorAll('.btn-view-student').forEach(btn => {",
  dirEventHandlers
);

// 10. Add health/attendance event calls in attach functions
content = content.replace(
